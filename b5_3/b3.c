...
void user_handler(int input_pipe[], int output_pipe[])	//Cai dat ham tien trinh cha --> du lieu cua nguoi dung 
{
	char c; int rc;
	close(input_pipe[1]);
	close(output_pipe[0]);
...
void translator(int input_pipe[], int output_pipe[]) //Cai dat ham xu ly du lieu cho tien trinh con
{
	char c; int rc;
	close(input_pipe[1]);
	close(output_pipe[0]);
...
int main()
{
	...
	rc = pipe(user_to_translator); //Duong ong mot --> gui data cha sang data con
	if(rc == -1)
	{
